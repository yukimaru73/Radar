...
TrackingRadar = {
	...
	isTracking = function(self)
		local mass_h =input.getNumber(self.ch_in_h+1)*input.getNumber(self.ch_in_h+2)
		local mass_v =input.getNumber(self.ch_in_v+1)*input.getNumber(self.ch_in_v+2)

		return input.getNumber(self.ch_in_h+1) ~= 0, input.getNumber(self.ch_in_v+1) ~= 0, (mass_v-mass_h)<0.25 , mass_h
	end;
	---@endsection
	...
	getPos = function(self)
		local x, y, z, xz, a, ith, itv, pi2, angle_h, angle_v, distance
		a, pi2  = self, math.pi * 2
		ith, itv = a:isTracking()
		if not (ith and itv) then return { 0, 0, 0 } end
		angle_h, angle_v, distance = input.getNumber(a.ch_in_h), input.getNumber(a.ch_in_v), input.getNumber(a.ch_in_h + 1)
		y = distance * math.sin(angle_h * pi2)
		xz = distance * math.cos(angle_h * pi2)
		x = xz * math.cos(angle_v * pi2)
		z = xz * math.sin(angle_v * pi2)
		return { x, y+0.5, z }
	end;
	---@endsection
	...
	trackingUpdate = function(self)
		local a = self
		local azim, elev, dist_h, dist_v =
		input.getNumber(a.ch_in_v),
		input.getNumber(a.ch_in_h),
		input.getNumber(a.ch_in_h+1),
		input.getNumber(a.ch_in_v+1)
